\section{Problem setup and notations}\label{sec:chap4 notation_setup}
...
It is convenient to view a district $v$ as a vector that live in $\mathcal{V}\subset\{0,1\}^P$, where 
\begin{align}
v_p =
    \begin{cases}
    ...
    \end{cases}
\end{align}
Let $\mathcal{W}:=\mathcal{V}^I/S_I$ be the space of unordered $I$-tuples from $\mathcal{V}$ and $\mathcal{X}\subset\mathcal{W}$ be the subset that forms plans.
A natural way to define a distance $\mathcal{V}\times\mathcal{V}\to\mathbb{R}$ between two districts $v$ and $v'$ is via a capped population-weighted $l^1$:
...
Given a cluster of districts $\mathcal{\tilde V}\subset \mathcal{V}$, we define the centroid $\tilde v$ to be the point in $\{0,1\}^P$ that minimizes average distance to all points in $\mathcal{\tilde V}$.
Under the distance definition \eqref{eq:dist_district}, letters are naturally zero-one valued even if we relax the centroid definition to be an optimization over $[0,1]^P$.
...
Using $K$ clusters, we obtain an alphabet of size $K$ and can use these letters to form words, which are unordered tuples of letters with fixed size $I$.
...
Let $\mathcal{S}\subset\mathcal{W}$ be this representative set of words, which forms a set of strata over the space of plans.
...
\end{align}
Based on our construction of the words, for each plan $x$ we only evaluate $\psi(x;s)$ on its $L$ nearest candidate words $w_1(x),\dots,w_L(x)\in \mathcal{S}$ and set it to zero otherwise.
We define $\psi$ by
\begin{align}\label{eq:psi_relative}
    \psi(x;s)=
    \begin{cases}
        \exp\!\big(-T\kappa(D(x,s))), & s\in\{w_1(x),\dots,w_L(x)\},\\
        0, & \text{otherwise,}
    \end{cases}
\end{align}
...
When $T=0$, the function $\psi$ assigns equal weights to the $L$ nearest words of a given plan $x$.
